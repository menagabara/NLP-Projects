...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    def add_layer(input_layer, units, name):
        rnn_layer = GRU(units, activation='relu', return_sequences=True,implementation=2, name=name)(input_layer)
...
def final_model(input_dim, filters, kernel_size, conv_stride,conv_border_mode, units, recur_layers, output_dim=29):
    ...
    max_pool = MaxPooling1D(pool_size=2, strides=2, padding='valid')(conv_1d) 
    
    # Add batch normalization layer
    bn_rnn = BatchNormalization(name='bn_conv_1d')(conv_1d)
    
    # Dropout 40%
    dropout = Dropout(0.4)(bn_rnn)

    
    # Add x recurrent layers
    for layer in range(recur_layers):
        
        # Add bidirectional layer
        bidir_rnn = Bidirectional(GRU(units, return_sequences=True, implementation=2))(dropout)
